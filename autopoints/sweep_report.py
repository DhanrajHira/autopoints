...
def discover_sweep_metadata(sweep_path: Path) -> list[Path]:
    ...
    return sorted({path.resolve() for path in resolved.rglob("simulation.meta.json")})
...
def collect_sweep_report(
    ...
    for metadata_path in metadata_paths:
        ...
        group = groups.setdefault(key, {"params": dict(record.params), "payload": {}})
    ...
    for spec in specs:
        matches = sorted(name for name in all_metric_names if spec.regex.search(name))
        if not matches:
            warnings.append(
