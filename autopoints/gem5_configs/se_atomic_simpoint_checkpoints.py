...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create AtomicSimpleCPU fast-forwarded SimPoint checkpoints."
    )
...
# AtomicSimpleCPU is the KVM-free fallback for hosts where KVM is unavailable.
# AtomicSimpleCPU is gem5's fastest functional CPU, and NoCache fast-forwards
# memory accesses straight to DRAM with no cache modeling, so this is the
# fastest checkpoint-generation setup that does not depend on KVM or perf.
